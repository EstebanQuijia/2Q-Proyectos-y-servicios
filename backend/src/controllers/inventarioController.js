...
exports.obtenerInventario = (req, res) => {
  const query = `
    SELECT 
      te.id, te.nombre, te.tipo, te.marca, te.modelo, te.descripcion, te.foto,
      COUNT(e.id) as total,
      SUM(CASE WHEN e.estado = 'disponible' AND e.activo = TRUE THEN 1 ELSE 0 END) as disponibles,
      STRING_AGG(e.id::text, ',') as unidades_ids
    FROM tipos_equipos te
    LEFT JOIN equipos e ON te.id = e.tipo_equipo_id
...
exports.obtenerUnidadesPorTipo = (req, res) => {
    const { tipoId } = req.params;
    const query = `
        SELECT id, numero_serie, estado, observaciones 
        FROM equipos 
        WHERE tipo_equipo_id = $1 AND activo = TRUE
        ORDER BY estado ASC, numero_serie ASC
    `;

    db.all(query, [tipoId], (err, rows) => {
        if (err) return res.status(500).json({ error: 'Error al obtener las unidades' });
        res.json(rows);
    });
};